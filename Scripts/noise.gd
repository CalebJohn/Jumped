...
func fbmsimplex(x):
	return simplex(x*0.02)*0.7+simplex(x*0.07)*0.3+simplex(x*0.005)

func curl(x):
	var d = derive(x)
	return Vector2(d.y, -d.x)
func derive(y):
	var x = Vector2(y.x, y.y)
	var ex = Vector2(0.01, 0)
	var ey = Vector2(0, 0.01)
	return (Vector2(simplex(x+ex)-simplex(x-ex), simplex(x+ey)-simplex(x-ey))).normalized()
func simplex(x):
	var n0 = 0
	var n1 = 0
	var n2 = 0
